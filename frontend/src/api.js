...
if (!import.meta.env.VITE_API_KEY) {
  console.warn("[api] VITE_API_KEY is not set — agent endpoint will return 401");
}
...
export const getMarketplace = (sort = "top_rated", limit = 20, offset = 0) =>
  api
    .get("/dashboard/marketplace", { params: { sort, limit, offset }, timeout: 15_000})
    .then((r) =>
      r.data.listings.map((b) => ({
        ...b,
...
export const getPlatformStats = () =>
  api.get("/dashboard/platform", {timeout: 15_000}).then((r) => {
    const d = r.data;
    return {
      ...d,
...
export const getLeaderboard = (limit = 10) =>
  api.get("/dashboard/leaderboard", { params: { limit } }).then((r) =>
    r.data.leaderboard.map((e) => ({
      ...e,
...
export const searchMemories = (q, top_k = 5) =>
  api
    .get("/memory/search", { params: { q, top_k } })
...
export const rateBundle = (bundleId, rating, wallet) =>
  api
    .post(`/dashboard/rate/${bundleId}`, {
      rating,
